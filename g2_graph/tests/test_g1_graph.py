...
class SmallSquareGraphTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for v in self.g1.get_vertex_iterator():
            self.g1.set_orientation(v, orientation)
...
class GetVertexCostTestCase(SmallSquareGraphTestCase):
    def runTest(self):
        ...
        self.assertTrue(len(vertex_cost) == self.g1_vertices_N)
...
class GetEdgeCombinationCostTestCase(SmallSquareGraphTestCase):
    def runTest(self):
        edge_combination_cost = self.g1.get_edge_combination_cost(comb_angle_factor=1.0)
        ...
        self.assertEqual(edge_combination_cost[(e_01, e_12)], np.pi/2.)
        self.assertEqual(edge_combination_cost[(e_12, e_23)], np.pi/2.)
        self.assertEqual(edge_combination_cost[(e_23, e_30)], np.pi/2.)
        self.assertEqual(edge_combination_cost[(e_01, e_30)], np.pi/2.)
