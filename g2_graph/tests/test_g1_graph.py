...
class AddEdgeTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        new_edge = self.g1.add_edge(2, 6)
        edge_index_map = self.g1.get_edge_index_map()

        self.assertEqual(edge_index_map[new_edge.source(), new_edge.target()], self.g1_edges_N - 1 + 1)
        self.assertEqual(self.g1.get_number_of_edges(), self.g1_edges_N + 1)

class GetVertexIndexMapTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        vertex_index_map = self.g1.get_vertex_index_map()
        
        for u in range(self.g1_vertices_N):
            self.assertEqual(vertex_index_map[u], u)

        # Test what happens if we add a vertex:
        v = self.g1.add_vertex()
        vertex_index_map = self.g1.get_vertex_index_map()
        for u in range(self.g1.get_number_of_vertices()):
            self.assertEqual(vertex_index_map[u], u)
...
class GetVertexIteratorTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        vertex_it = self.g1.get_vertex_iterator()
...
class GetEdgeIteratorTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        edge_it = self.g1.get_edge_iterator()
        
        for e in edge_it:
            self.assertEqual(e, self.g1.get_edge(e.source(), e.target()))

class GetVertexArrayTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        vertex_array = self.g1.get_vertex_array()
        self.assertTrue(np.all(vertex_array == np.arange(self.g1_vertices_N)))
        
        self.g1.add_vertex()
        vertex_array = self.g1.get_vertex_array()
        self.assertTrue(np.all(vertex_array == np.arange(self.g1_vertices_N + 1)))

class GetEdgeArrayTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        edge_array = self.g1.get_edge_array()
        #Test array type
        ...
        edge_array_new = self.g1.get_edge_array()
        ...
        edge_array_reindex = self.g1.get_edge_array()
...
class NewVertexPropertyTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        vp = self.g1.new_vertex_property("orientation", "vector<double>")
...
class SetGetVertexPropertyTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        ...
        self.g1.set_vertex_property("orientation", 1, orientation)
        self.assertTrue(np.all(self.g1.get_vertex_property("orientation", 1) == orientation))

        with self.assertRaises(KeyError):
            self.g1.get_vertex_property("diesdas", 1)

        with self.assertRaises(ValueError):
            self.g1.get_vertex_property("orientation", 100)

class NewEdgePropertyTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        ep = self.g1.new_edge_property("distance", "double")
...
class SetGetEdgePropertyTestCase(SmallCircleGraphTestCase):
    def runTest(self):
        ep = self.g1.new_edge_property("distance", "double")
        distance = 12.5555
        self.g1.set_edge_property("distance", 1, 2, distance)
        self.assertTrue(self.g1.get_edge_property("distance", 1, 2) == distance)

        with self.assertRaises(KeyError):
            self.g1.get_edge_property("diesdas", 1, 2)
