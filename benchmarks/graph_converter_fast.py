...
class GraphConverterFast:
    ...
    def get_mapping_fast(self):
        ...
        ea_noidx = edge_array.T[:2].T
        vertex_array = self.g1.get_vertex_array()
        partner = self.g1.get_vertex_property("partner").a

        for i in range(len(vertex_array)):
            v = vertex_array[i] 
            inc_edges = edge_array[np.where(ea_noidx == v)[0]] 
